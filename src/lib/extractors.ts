...
const SMUGMUG_SIZES = ["X5", "X4", "X3", "X2", "XL", "L", "X"];

async function resolveSmugMug(url: string): Promise<string> {
  // Strip trailing /A, /0/SIZE/filename, or /A/0/SIZE/filename to get the base image path
  const base = url
    .replace(/\/A\/0\/[^/]+\/[^/]+$/, "")
    .replace(/\/0\/[^/]+\/[^/]+$/, "")
    .replace(/\/A$/, "");
  ...
  for (const size of SMUGMUG_SIZES) {
    const candidate = `${base}/0/${size}/photo.jpg`;
    try {
  ...
  // Fallback: scrape og:image from the page
  return resolveToDirectImage(url);
}
